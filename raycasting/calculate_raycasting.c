...
void	calculate_step_and_initial_sidedist(t_cast *cast, t_map img)
{
	...
	{
		cast->stepX = -1;
		cast->sideDistX = (double)(img.posX - (double)cast->mapX) * cast->deltaDistX;
	}
	else
	{
		cast->stepX = 1;
		cast->sideDistX = ((double)cast->mapX + 1.0 - img.posX) * cast->deltaDistX;
	}
	if (cast->rayDirY < 0.0)
	{
		cast->stepY = -1;
	cast->sideDistY = (img.posY - (double)cast->mapY) * cast->deltaDistY;
	}
	else
	{
		cast->stepY = 1;
		cast->sideDistY = ((double)cast->mapY + 1.0 - img.posY) * cast->deltaDistY;
	}
}


/* calcule le pixel le plus bas et le plus haut pour remplir la bande actuell */
